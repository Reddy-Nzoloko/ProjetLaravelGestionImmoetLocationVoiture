<x-modal name="create-listing" focusable>
    ...
    @endphp
        ...
        <header>
            @php
                $sectorLabel = $sector === 'auto' ? 'Secteur Automobile' : ($sector === 'immo' ? 'Secteur Immobilier' : ($sector === 'vetement' ? 'Secteur Habillement' : ucfirst($sector)));
            @endphp
            ...
            <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100 flex items-center">
                ...
                {{ __('Nouvelle Annonce') }} 
                <span class="ms-2 text-sm font-normal text-gray-500">({{ $sectorLabel }})</span>
            </h2>
        </header>
        ...
        <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="col-span-1">
                ...
                <x-text-input id="title" name="title" type="text" class="mt-1 block w-full" 
                    placeholder="{{ $sector === 'auto' ? 'Ex: Toyota RAV4...' : ($sector === 'immo' ? 'Ex: Villa 5 chambres...' : 'Ex: Robe traditionnelle, ensemble...') }}" required />
            </div>
            <div class="col-span-1">
        ...
        <div class="mt-6 p-5 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl">
            ...
            <div x-show="category === 'vetement'">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <x-input-label value="Marque / Marque de couture" />
                        <x-text-input name="brand" type="text" class="mt-1 block w-full" placeholder="Ex: Wax Couture" />
                    </div>
                    <div>
                        <x-input-label value="Taille / Référence" />
                        <x-text-input name="size" type="text" class="mt-1 block w-full" placeholder="Ex: M, L, 38" />
                    </div>
                </div>
                <div class="mt-4">
                    <x-input-label value="État" />
                    <select name="condition" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 rounded-md">
